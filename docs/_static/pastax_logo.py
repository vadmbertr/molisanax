...
RAMP       = 0.2   # fraction of curve over which offset ramps up from IC
...
def plot():
    ...
    x_ref, y_ref = offset_curve(x_ref, y_ref, 0, seed=SEED_REF, noise_amp=NOISE_AMP / 2)
    x_l, y_l = offset_curve(x_ref, y_ref, -OFFSET, seed=SEED_LEFT)
    x_r, y_r = offset_curve(x_ref, y_ref, +OFFSET, seed=SEED_RIGHT)

    strands = [
        (x_l,   y_l,   LEFT_COLOR),
        (x_ref, y_ref, REF_COLOR),
        (x_r,   y_r,   RIGHT_COLOR),
    ]

    all_x = np.concatenate([x_ref, x_l, x_r])
    all_y = np.concatenate([y_ref, y_l, y_r])
    ...
    for x, y, color in strands:
        ax.plot(x, y, color=color, lw=LINEWIDTH, alpha=0.92,
